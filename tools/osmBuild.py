...
SUMO_HOME = os.environ.get(
    "SUMO_HOME", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..")
)
vclassRemove = {
    "passenger": ["--keep-edges.by-vclass", "passenger"],
    "publicTransport": [
        "--keep-edges.by-vclass",
        "passenger,bus,tram,rail_urban,rail",
    ],
    "road": [
        "--remove-edges.by-vclass",
        "tram,rail_urban,rail_electric,bicycle,pedestrian",
    ],
    "all": [],
}
...
optParser = sumolib.options.ArgumentParser(
    description="Import a OpenStreetMap file into SUMO"
)
...
optParser.add_argument(
    ...
    "--osm-file",
    category="input",
    help="full name of the osm file to import",
)
optParser.add_argument(
    "-m",
    "--typemap",
...
optParser.add_argument(
    ...
    category="processing",
    default="all",
    help="[(%s)]extract network for a reduced set of vehicle classes"
    % possibleVClassOptions,
)
optParser.add_argument(
    "-d",
    "--output-directory",
...
optParser.add_argument(
    ...
    "--netconvert-options",
    category="processing",
    default=DEFAULT_NETCONVERT_OPTS,
    help="comma-separated options for netconvert",
)
optParser.add_argument(
    "--pedestrians",
    category="processing",
    action="store_true",
    default=False,
    help="add pedestrian infrastructure to the network",
)
optParser.add_argument(
    "-y",
    "--polyconvert-options",
    category="processing",
    default="-v,--osm.keep-full-type",
...
optParser.add_argument(
    ...
    action="store_true",
    default=False,
    help="enable verbose netconvert output",
)
...
def build(args=None, bindir=None):
    ...
    if (options.oldapi_prefix and options.osm_file) or not (
        options.oldapi_prefix or options.osm_file
    ):
        optParser.error(
            "exactly one of the options --osm-file and --oldapi-prefix must be supplied"
        )
    if options.typemap and not os.path.isfile(
        options.typemap.replace("${SUMO_HOME}", SUMO_HOME)
    ):
    ...
    if options.vehicle_classes not in vclassRemove:
        optParser.error(
            'invalid vehicle class "%s" given' % options.vehicle_classes
        )
    if not os.path.isdir(options.output_directory):
        optParser.error(
            'output directory "%s" does not exist' % options.output_directory
        )
    ...
    netconvertOpts += [
        "--"
        + opt[: opt.find(" ") + 1]
        + opt[opt.find(" ") + 1 :].replace(" ", ",")
        for opt in options.netconvert_options.strip().split(",")
    ] + ["--osm-files"]
    polyconvertOpts = (
        [polyconvert]
    ...
    if prefix:  # used old API
        num = options.tiles
        tiles = ",".join(
            ["%s%s_%s.osm.xml" % (prefix, i, num) for i in range(num)]
        )
    ...
    netconvertOpts = [
        getRelative(options.output_directory, o) for o in netconvertOpts
    ]
    subprocess.call(
        netconvertOpts + ["--save-configuration", cfg],
    ...
    if options.typemap:
        ...
        # use relative paths where possible
        polyconvertOpts = [
            getRelative(options.output_directory, o) for o in polyconvertOpts
        ]
        subprocess.call(
            polyconvertOpts + ["--save-configuration", cfg],
            cwd=options.output_directory,
        )
